...
- [Env quirks Replit (main agent)](replit-env-quirks.md) — git mv bloqué pour le main agent : utiliser `mv` simple (op. FS), le commit auto gère le versioning.
- [Web portal publish constraint](web-portal-publish.md) — static portal dist can't reach mobile :5173 / backend :8000 once published; live link only works during a dev session.
