...
## Long-running ML training: host it in a workflow, detached bg gets reaped
Training a couple is a multi-minute job. A detached background process
(`nohup`/`setsid python -m app.ml.train ... &`) is UNRELIABLE on Replit: the agent
tool-call harness reaps it between calls, so it dies mid-run with an EMPTY log and no
models — which LOOKS like OOM but is NOT.
**Why:** it is process-lifecycle, not memory — confirm with `/sys/fs/cgroup/memory.events`
(`oom_kill 0`) and `/sys/fs/cgroup/memory.peak` (cap ~8GB; a single couple peaks <5GB).
**How to apply:** run one-shot ML regen inside a temporary **console workflow** (the
platform keeps workflow processes alive); full cores, no taskset/thread caps needed.
Regen order = seed → build_dataset → train → generate_global; registry keeps ONE active
model per (target,horizon). Helper: `backend/scripts/regen_hyper.sh`.
