...
> **⚠️ Document historique (legacy archivé — Phase 8.5.2).** Concerne **uniquement**
> l'ancienne app React/Vite, désormais **archivée** dans
> `archive/legacy-web/MedAICare_V.3_10Patients/` (non active, non publiée). Le web
> **actif** est le nouveau portail public `web-portal/` (présentation + accès mobile,
> sans login/ML/token) — voir `docs/web/WEB_PORTAL_STRATEGY.md` et `web-portal/README.md`.
> Le backend FastAPI (`backend/`, port 8000) et le mobile Expo (`mobile/`, port 5173)
> sont documentés dans `docs/migration/`, `docs/api/`, `docs/security/`, `docs/ops/`,
> `docs/compliance/` et `docs/mobile/`.
...
## Structure du projet
Le code source (legacy archivé) vit dans `archive/legacy-web/MedAICare_V.3_10Patients/` :
...
## Lancement & déploiement (legacy archivé — référence uniquement)
> Ce dossier **n'est plus câblé** aux workflows ni au déploiement depuis la Phase 8.5.2.
> Le workflow « Start application » (port 5000) et la publication statique pointent
> désormais vers `web-portal/`.
- **Dev (référence) :** `cd archive/legacy-web/MedAICare_V.3_10Patients && npm run dev`.
- **Build (référence) :** `cd archive/legacy-web/MedAICare_V.3_10Patients && npm run build`.
- Déploiement : site statique (désormais `web-portal/dist`).
