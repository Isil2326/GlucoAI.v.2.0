...
IoMT + XAI). Trois couches : **portail web institutionnel** React indépendant
(`web-portal/`, présentation/soutenance/accès mobile, **sans login/ML/token**),
socle backend FastAPI (`backend/`, source de vérité), **app mobile Expo**
(`mobile/`, application principale patient/clinicien). L'ancien web est **archivé**
(`archive/legacy-web/MedAICare_V.3_10Patients/`, non actif/non publié — Phase 8.5.2).
...
- Voir limites complètes : `archive/legacy-web/MedAICare_V.3_10Patients/LIMITATIONS.md`.

## Stack
- **Web (portail `web-portal/`) :** React 19 · TypeScript · Vite 7 · Tailwind 4 ·
  `vite-plugin-singlefile` · `qrcode` (port 5000 ; présentation + accès mobile, aucun calcul métier).
...
| 8.5.2 | Séparation web/mobile : archivage legacy + nouveau `web-portal/` indépendant | **livré, à valider** |
| 8.6 | Posture produit portail web (ton premium medtech, limites regroupées ~80/20) | **livré, à valider** |
| 8.6.1 | Refonte premium portail (« Signal clair ») + aperçu app live embarqué (iframe) | **livré, à valider** |
...
cd web-portal && npm run dev                                    # « Start application » (portail web, 5000)
```
...
- **Start application** — **portail web `web-portal/`** React/Vite, port 5000
  (preview principal `webview` ; déploiement site statique → `web-portal/dist/`).
  C'est le rendu par défaut du preview Replit. L'app **mobile** est sur le **port 5173**
  (workflow « Mobile App », `console`) — lien/QR « Ouvrir l'app mobile » depuis le portail.
  L'ancien web (`archive/legacy-web/MedAICare_V.3_10Patients/`) n'est **plus** câblé à
  aucun workflow ni au déploiement (Phase 8.5.2).
...
- Phase 8.5.2 : `RAPPORT_PHASE_8_5_2.md` (séparation web/mobile : archivage legacy + `web-portal/`)
...
**Frontend web** — **portail actif** : `web-portal/README.md` ·
`docs/web/WEB_PORTAL_STRATEGY.md` · **legacy archivé** :
`archive/legacy-web/README.md` · `docs/frontend/REACT_APP_ARCHITECTURE.md`
(historique clinicien v6.0, design system — décrit l'ancien web archivé)
